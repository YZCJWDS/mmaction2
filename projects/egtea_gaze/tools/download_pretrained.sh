...
if [ -f "$SLOWFAST_FILE" ]; then
    echo "[OK] SlowFast-R50 权重已存在 ($(du -h "$SLOWFAST_FILE" | cut -f1))"
else
...
TSM_RESNET_FILE="$TORCH_CACHE/resnet50-0676ba61.pth"
...
echo "预训练权重准备完成！"
echo ""
echo "SlowFast: $SLOWFAST_FILE"
echo "ResNet50: $TSM_RESNET_FILE"
