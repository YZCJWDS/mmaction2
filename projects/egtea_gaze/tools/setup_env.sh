...
if [ -z "$CUDA_VER" ] && command -v nvcc &> /dev/null; then
    ...
    [ -n "$CUDA_VER" ] && echo "  从 nvcc 检测 CUDA: $CUDA_VER"
fi

if [ -z "$CUDA_VER" ]; then
    CUDA_VER="11.8"
    echo "  [INFO] 无法检测 CUDA 版本，默认使用 $CUDA_VER"
...
echo "  3. bash projects/egtea_gaze/tools/download_pretrained.sh"
echo "  4. python projects/egtea_gaze/tools/smoke_test.py --config projects/egtea_gaze/configs/tsm_r50_egtea.py"
